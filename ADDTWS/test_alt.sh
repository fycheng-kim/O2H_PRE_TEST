#!/bin/bash
log_path=tws_alt_test.log
pwd=


echo "======== TEST CASE 1: SHOULD SUCCESS ===========" >> $log_path
sshpass -p $pwd ssh twsuser@172.19.250.22 <<END_SCRIPT &>>$log_path
	conman "altpri FODSETL#F2H_MOMO_ORDER_D(1800 06/30/22);pri=30;noask" & echo "TWS_ALT_STS: $?"
END_SCRIPT
echo ""
...
sshpass -p $pwd ssh twsuser@172.19.250.22 <<END_SCRIPT &>>$log_path
	conman "altpri FODSETL#F2H_MOMO_ORDER_DXXX(1800 06/30/22);pri=30;noask" & echo "TWS_ALT_STS: $?"
END_SCRIPT
echo ""
...
sshpass -p $pwd ssh twsuser@172.19.250.22 <<END_SCRIPT &>>$log_path
	conman "altpri FODSETL#F2H_MOMO_ORDER_D(1800 22/06/30);pri=30;noask" & echo "TWS_ALT_STS: $?"
END_SCRIPT
echo ""
...
sshpass -p $pwd ssh twsuser@172.19.250.22 <<END_SCRIPT &>>$log_path
	conman "altpri FODSETL#F2H_MOMO_ORDER_D(1800 06/30/22);pri=300;noask" & echo "TWS_ALT_STS: $?"
END_SCRIPT
echo ""
